...
<div class="container mx-auto">
    ...
    <div class="mb-6">
        ...
        <div class="flex justify-between items-center">
            <a href="{{ route('admin.program-news.create') }}" class="px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:from-blue-700 hover:to-blue-800 transition-all duration-200 shadow-lg hover:shadow-xl flex items-center">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        <td class="px-4 py-3 text-sm">
                            ...
                            @if($program->tag)
                                <span class="text-xs bg-orange-100 text-orange-800 px-2 py-0.5 rounded mt-1 inline-block">{{ $program->tag }}</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-sm">
                            <span class="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded capitalize">{{ $program->type }}</span>
                        </td>
                        <td class="px-4 py-3 text-sm">
                            <span class="bg-purple-100 text-purple-800 text-xs px-2 py-1 rounded capitalize">{{ $program->major }}</span>
                        </td>
                        <td class="px-4 py-3 text-sm">
                            <span class="bg-green-100 text-green-800 text-xs px-2 py-1 rounded uppercase">{{ $program->level }}</span>
                        </td>
                        ...
                        <td class="px-4 py-3 text-sm">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('admin.program-news.edit', ['program_news' => $program->id]) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded text-xs transition-colors">
                                    ✏️ Edit
                                </a>
                                <form action="{{ route('admin.program-news.destroy', ['program_news' => $program->id]) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus program ini?')">
                                    @csrf
                                    @method('DELETE')
