...
<div class="container mx-auto max-w-4xl">
    ...
    <div x-data="{ show: true }" x-show="show" x-transition.duration.500ms class="mb-8 flex items-center justify-between bg-red-100 border border-red-400 text-red-700 px-6 py-4 rounded-lg shadow-lg">
        <div class="flex items-center">
            ...
            <div>
                <span class="font-semibold">Gagal mengupdate program:</span>
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-2">Edit Program</h1>
        <p class="text-gray-600">Update informasi program</p>
    </div>

    <div class="bg-white rounded-xl shadow-lg p-8">
        <form action="{{ route('admin.program-news.update', ['program_news' => $program->id]) }}" method="POST" 
            x-data="{ features: {{ json_encode(old('features', $program->features)) }}, loading: false }" @submit="loading = true">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="tag" value="{{ old('tag', $program->tag) }}" placeholder="Contoh: Terlaris"
                ...
                <div>
                    ...
                    <input type="text" name="card_type" value="{{ old('card_type', $program->card_type) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="sold_count" value="{{ old('sold_count', $program->sold_count) }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                </div>
                ...
                <div>
                    ...
                    <select name="type" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 @error('type') border-red-500 @enderror">
                        <option value="">Pilih Tipe</option>
                        <option value="bimbel" {{ old('type', $program->type) == 'bimbel' ? 'selected' : '' }}>Bimbel</option>
                        <option value="tryout" {{ old('type', $program->type) == 'tryout' ? 'selected' : '' }}>Try Out</option>
                        <option value="bundle" {{ old('type', $program->type) == 'bundle' ? 'selected' : '' }}>Bundle</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="level" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 @error('level') border-red-500 @enderror">
                        <option value="">Pilih Level</option>
                        <option value="d3" {{ old('level', $program->level) == 'd3' ? 'selected' : '' }}>D3</option>
                        <option value="profesi" {{ old('level', $program->level) == 'profesi' ? 'selected' : '' }}>Profesi</option>
                ...
                <div>
                    ...
                    <select name="is_active" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
            ...
            <div class="flex gap-4 mt-8">
                <button type="submit" :disabled="loading"
                    ...
                    <span x-show="!loading">Update Program</span>
                    <span x-show="loading">Mengupdate...</span>
                </button>
