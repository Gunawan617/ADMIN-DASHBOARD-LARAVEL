    ...
    <div class="container mx-auto max-w-4xl">
        ...
        <div class="bg-white rounded-xl shadow-lg p-8">
            <form action="{{ route('admin.program-news.update', ['program_news' => $program->id]) }}" method="POST"
                x-data="{ features: {{ json_encode(old('features', $program->features)) }}, loading: false }"
                @submit="loading = true">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="type" required
                            ...
                            <option value="bimbel" {{ old('type', $program->type) == 'bimbel' ? 'selected' : '' }}>Bimbel
                            </option>
                            <option value="tryout" {{ old('type', $program->type) == 'tryout' ? 'selected' : '' }}>Try Out
                            </option>
                            <option value="bundle" {{ old('type', $program->type) == 'bundle' ? 'selected' : '' }}>Bundle
                            </option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="major" list="major-list" value="{{ old('major', $program->major) }}"
                            required placeholder="Ketik atau pilih jurusan"
                    ...
                    <div>
                        ...
                        <select name="is_active"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                            <option value="1" {{ old('is_active', $program->is_active) == 1 ? 'selected' : '' }}>Aktif
                            </option>
                            <option value="0" {{ old('is_active', $program->is_active) == 0 ? 'selected' : '' }}>Nonaktif
                            </option>
                        </select>
                    </div>
                </div>
